...
if ($_POST) {
    ...
    if (empty($batas_waktu || $tanggal_pembayaran || $status || $status_pembayaran)) {
        echo "<script>alert('Harap semua data diisi dengan benar!');location.href='../html/index-kasir.php';</script>";
    } else {
        ...
        if ($update_transaksi) {
            echo "<script>alert('Ubah Data Transaksi Sukses!');location.href='../html/index-kasir.php';</script>";
        } else {
            echo "<script>alert('Ubah Data Transaksi Gagal! silakan coba kembali!');location.href='../html/index-kasir.php';</script>";
        }
    };
}