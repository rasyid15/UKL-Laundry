...
if ($_POST) {
    ...
    if (empty($member) || empty($tanggal) || empty($batas_waktu) || empty($tanggal_pembayaran) || empty($status) || empty($status_pembayaran)) {
        echo "<script>alert('Harap semua data diisi dengan benar!');location.href='../html/order-kasir.php';</script>";
    } else {
        ...
        if ($insert_transaksi) {
            echo "<script>alert('Transaksi Berhasil');location.href='../html/index-kasir.php';</script>";
        } else {
            echo "<script>alert('Transaksi Gagal! silakan coba kembali!');location.href='../html/order-kasir.php';</script>";
        };
    }
    ...
    if ($insert_dtl_transaksi) {
        echo "<script>alert('Transaksi Sukses!');location.href='../html/order-kasir.php';</script>";
    } else {
        echo "<script>alert('Transaksi Gagal! silakan coba kembali!');location.href='../html/order-kasir.php';</script>";
    };
};